...
const (
	...
	_prefixMidsOnline = "ol_mids"  // set -> mids
	_prefixMidsMatchOnline = "ol_match_mids"  // set -> mids
)
...
func keyMidsOnline() string {
	return fmt.Sprintf(_prefixMidsOnline)
}

func keyMidsMatchOnline() string {
	return fmt.Sprintf(_prefixMidsMatchOnline)
}
...
func (d *Dao) AddMapping(c context.Context, mid int64, key, server string) (err error) {
	...
	if mid > 0 {
		...
		if err = conn.Send("SADD", keyMidsOnline(), mid); err != nil {
			log.Errorf("conn.Send(SADD %d) error(%v)", mid, err)
			return
		}
		n += 3
	}
...
func (d *Dao) AddMatchOnline(c context.Context, mid int64, key, server string) (err error) {
	conn := d.redis.Get()
	defer conn.Close()
	var n = 0
	if mid > 0 {
		if err = conn.Send("SADD", keyMidsMatchOnline(), mid); err != nil {
			log.Errorf("conn.Send(SADD %d) error(%v)", mid, err)
			return
		}
		n += 1
...
func (d *Dao) DelMapping(c context.Context, mid int64, key, server string) (has bool, err error) {
	...
	if mid > 0 {
		...
		if err = conn.Send("SREM", keyMidsOnline(), mid); err != nil {
			log.Errorf("conn.Send(SREM %d) error(%v)", mid, err)
			return
		}
		n += 2
	}
...
func (d *Dao) DelMatchOnline(c context.Context, mid int64, key, server string) (has bool, err error) {
	conn := d.redis.Get()
	defer conn.Close()
	n := 0
	if mid > 0 {
		if err = conn.Send("SREM", keyMidsMatchOnline(), mid); err != nil {
			log.Errorf("conn.Send(SREM %d) error(%v)", mid, err)
			return
		}
		n += 1
...
func (d *Dao) PopMids(c context.Context, num int64) (res []int64 ,err error) {
	conn := d.redis.Get()
	defer conn.Close()
	var count int64
	matchMidKey := keyMidsMatchOnline()
	if count, err = redis.Int64(conn.Do("SCARD", matchMidKey)); err != nil {
		log.Errorf("conn.Do(SCARD %s) error(%v)", matchMidKey, err)
		return
	}
	if count < num {
		return
	}
	if res, err = redis.Int64s(conn.Do("SPOP", matchMidKey, num)); err != nil {
		log.Errorf("conn.Do(SPOP %s %d) error(%v)", matchMidKey, num, err)
		return
	}
	return
}
